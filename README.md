...
PostgreSQL and serves a fast, filterable browse UI. Phase 2 (LLM analysis of
project suitability) has a schema seam in place but is intentionally not built.
...
- **Browsing** is served entirely from the database and never calls arXiv.
...
- Unit tests parse a saved **real** arXiv Atom response (fixtures are fine in
  tests; only the running app is restricted to live data).
- Integration tests host the real API over in-memory Sqlite and cover browse
  filtering/sorting/paging/validation, admin auth (404/401/202), and
  double-run upsert idempotency. The arXiv client is stubbed in tests so they
...
- `AnalysisResults` table exists now: 1:1 with `Papers` (unique FK),
  `SchemaVersion`, `Model`, `ResultJson` (`text` for provider portability),
  denormalized `CompositeScore` for sorting.
- `IAnalysisService` is defined in Application with **no implementation and no
  registration**; its `AnalysisRequest(CategoryCode, MaxPapers, Since)` shape
  forces analysis of bounded, filtered subsets — the whole corpus can never be
  analyzed in one call.
- No LLM package is referenced anywhere in the solution.
